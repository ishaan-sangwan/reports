...
def stock():
    ...
    source_locations = grn['Source Location'].unique()
    ...
    for location in source_locations:
        report_row = {'Source Location': location}
        for i in range(6):
            ...
            filtered_grn = grn[(grn['Source Location'] == location) & (grn['Grn Date'].dt.month == month) & (grn['Grn Date'].dt.year == year)]
    ...
    # Location selection
    loc = st.selectbox('Select a Location', options=['Select'] + list(grn['Source Location'].unique()))

    if loc == 'Select':
        # If 'Select' is chosen, sum across all locations
        plot_data = report_df.set_index('Source Location').sum(axis=0).reset_index()
        plot_data.columns = ['Month', 'Total Dispatch Weight']
    else: 
        # Filter the report_df for the selected location
        plot_data = report_df[report_df['Source Location'] == loc].sum(numeric_only=True).reset_index()
        plot_data.columns = ['Month', 'Total Dispatch Weight']
    
    # Remove the "Source Location" from the plot_data for plotting
    plot_data = plot_data[plot_data['Month'] != 'Source Location']
